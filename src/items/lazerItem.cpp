...
LazerItem :: LazerItem(TextureOb* _pTo_itemTexOb, int _damage_orig, int _radius_orig, int _modules_num_max, int _mass, int _condition_max, int _deterioration_rate)
{
...
void LazerItem :: updatePropetries()
{
...
void LazerItem :: updateOwnerPropetries()
{ 
    //(*pTo_owner).updateFireAbility();
}


void LazerItem :: updateInfo()
{   
...
std::string LazerItem :: returnDamageStr()
{
      if (damage_add == 0)
...
std::string LazerItem :: returnRadiusStr()
{
       if (radius_add == 0)
...
bool LazerItem :: insertModule(LazerModule* pTo_lazerModule)
{
...
LazerItem* lazerGenerator(int race_id, int revision_id = -1)
{
    if (race_id == -1)
    ...
    TextureOb* pTo_itemTexOb = g_TEXTURE_MANAGER.returnPointerToRandomTexObFromList(&g_TEXTURE_MANAGER.lazerItem_texOb_pList);     
    ...
    LazerItem* pTo_lazer = new LazerItem(pTo_itemTexOb, damage_orig, radius_orig, modules_num_max, mass, condition_max, deterioration_rate);
    return pTo_lazer;
}
