...
RadarItem :: RadarItem(TextureOb* _pTo_itemTexOb, int _radius_orig, int _modules_num_max, int _mass, int _condition_max, int _deterioration_rate)
{
...
void RadarItem :: updatePropetries()
{
    radius = radius_orig + radius_add;
}

void RadarItem :: countPrice()
{
...
void RadarItem :: updateOwnerPropetries()
{
...
std::string RadarItem :: returnRadiusStr()
{
    if (radius_add == 0)
...
bool RadarItem :: insertModule(RadarModule* pTo_radarModule)
{
    if (modules_pList.size() < modules_num_max)
    {
        radius_add += pTo_radarModule->radius_add;
    
        updatePropetries();
        updateInfo();     // ??
        
        texOb_modules_pList.push_back(pTo_radarModule->pTo_texOb);
...
RadarItem* radarGenerator(int race_id, int revision_id = -1)
{
    if (race_id == -1)
    ...
    TextureOb* pTo_itemTexOb = g_TEXTURE_MANAGER.returnPointerToRandomTexObFromList(&g_TEXTURE_MANAGER.radarItem_texOb_pList);   
    ...
    RadarItem* pTo_radar = new RadarItem(pTo_itemTexOb, radius_orig, modules_num_max, mass, condition_max, deterioration_rate);
    return pTo_radar;
}
