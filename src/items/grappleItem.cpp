...
GrappleItem :: GrappleItem()
{}

GrappleItem :: GrappleItem(TextureOb* _pTo_itemTexOb, int _strength_orig, int _radius_orig, int _speed_orig, int _maxNumItem_orig, int _modules_num_max, int _mass, int _condition_max, int _deterioration_rate)   // joun strength and speed attributes into one
{
...
void GrappleItem :: updatePropetries()
{
...
void GrappleItem :: updateOwnerPropetries()
{
    //      self.owner.updateGrabAbility()
}


void GrappleItem :: updateInfo()
{
...
std::string GrappleItem :: returnStrengthStr()
{
     if (strength_add == 0)
...
std::string GrappleItem :: returnRadiusStr()
{
     if (radius_add == 0)
...
std::string GrappleItem :: returnSpeedStr()
{
     if (speed_add == 0)
...
std::string GrappleItem :: returnMaxNumItemStr()
{
     if (maxNumItem_add == 0)
...
bool GrappleItem :: insertModule(GrappleModule* pTo_grappleModule)
{
...
GrappleItem* grappleGenerator(int race_id, int revision_id = -1)
{
    if (race_id == -1)
    ...
    TextureOb* pTo_itemTexOb = g_TEXTURE_MANAGER.returnPointerToRandomTexObFromList(&g_TEXTURE_MANAGER.grappleItem_texOb_pList);   
    ...
    GrappleItem* pTo_grapple = new GrappleItem(pTo_itemTexOb, strength_orig, radius_orig, speed_orig, maxNumItem_orig, modules_num_max, mass, condition_max, deterioration_rate);
    return pTo_grapple;
}
