...
FreezerItem :: FreezerItem(TextureOb* _pTo_itemTexOb, int _freeze_orig, int _modules_num_max, int _mass, int _condition_max, int _deterioration_rate)
{
...
void FreezerItem :: updatePropetries()
{
     freeze = freeze_orig + freeze_add;
}


void FreezerItem :: countPrice()
{
...
void FreezerItem :: updateOwnerPropetries()
{
    //     self.owner.updateFreezeAbility()
}

void FreezerItem :: updateInfo()
{
...
std::string FreezerItem :: returnFreezeStr()
{
     if (freeze_add == 0)
...
bool FreezerItem :: insertModule(FreezerModule* pTo_freezerModule)
{
...
FreezerItem* freezerGenerator(int race_id, int revision_id = -1)
{
    if (race_id == -1)
    ...
    TextureOb* pTo_itemTexOb = g_TEXTURE_MANAGER.returnPointerToRandomTexObFromList(&g_TEXTURE_MANAGER.freezerItem_texOb_pList);    // FAKE TEXTURE OB LIST IS USED HERE
    ...
    FreezerItem* pTo_freezer = new FreezerItem(pTo_itemTexOb,  freeze_orig, modules_num_max, mass, condition_max, deterioration_rate);
    return pTo_freezer;
}
