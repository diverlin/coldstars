...
RocketItem :: RocketItem(TextureOb* _pTo_itemTexOb, int _ammo_max_orig, int _damage_orig, int _radius_orig, int _modules_num_max, int _mass, int _condition_max, int _deterioration_rate)
{
        CommonForItems_init(_pTo_itemTexOb, _modules_num_max, _mass, _condition_max, _deterioration_rate);
        
        type_id = WEAPON_ID;
        subtype_id = ROCKET_ITEM_ID;
...
void RocketItem :: updatePropetries()
{
...
void RocketItem :: updateOwnerPropetries()
{      
    pTo_ship->updateFireAbility();
}

std::string RocketItem :: returnAmmoStr()
{
    if (ammo_max_add == 0)
...
std::string RocketItem :: returnDamageStr()
{
    if (damage_add == 0)
...
std::string RocketItem :: returnRadiusStr()
{
    if (radius_add == 0)
...
bool RocketItem :: insertModule(RocketModule* pTo_rocketModule)
{
...
RocketItem* rocketGenerator(int race_id, int revision_id = -1)
{
    if (race_id == -1)
    ...
    TextureOb* pTo_itemTexOb = g_TEXTURE_MANAGER.returnPointerToRandomTexObFromList(&g_TEXTURE_MANAGER.rocketItem_texOb_pList);    
    ...
    RocketItem* pTo_rocket = new RocketItem(pTo_itemTexOb,  ammo_max_orig, damage_orig, radius_orig, modules_num_max, mass, condition_max, deterioration_rate);
    return pTo_rocket;
}
