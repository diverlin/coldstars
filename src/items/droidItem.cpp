...
DroidItem :: DroidItem(TextureOb* _pTo_itemTexOb, int _repair_orig, int _modules_num_max, int _mass, int _condition_max, int _deterioration_rate)
{
...
void DroidItem :: updatePropetries()
{
     repair = repair_orig + repair_add;
}

void DroidItem :: countPrice()
{
...
void DroidItem :: updateOwnerPropetries()
{
     //self.owner.updateRepairAbility()
}

std::string DroidItem :: returnRepairStr()
{
    if (repair_add == 0)
...
bool DroidItem :: insertModule(DroidModule* pTo_droidModule)
{
...
DroidItem* droidGenerator(int race_id, int revision_id = -1)
{
    if (race_id == -1)
    ...
    TextureOb* pTo_itemTexOb = g_TEXTURE_MANAGER.returnPointerToRandomTexObFromList(&g_TEXTURE_MANAGER.droidItem_texOb_pList);    
    ...
    DroidItem* pTo_droid = new DroidItem(pTo_itemTexOb, repair_orig, modules_num_max, mass, condition_max, deterioration_rate);
    return pTo_droid;
}
