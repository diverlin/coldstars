...
void MacroScenarioSelfSafety :: update_inStatic(Npc* _npc) const
{
	...
	{
		if (_npc->getStateMachine()->getCurrentMacroTask()->getTarget()->getTypeId() != ENTITY::PLANET)
		{ 
	...
	{
		if (_npc->getStateMachine()->getCurrentMicroTask()->getTarget()->getTypeId() != ENTITY::STARSYSTEM)
		{
