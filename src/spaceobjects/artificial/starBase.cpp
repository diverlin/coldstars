...
StarBase :: StarBase(int race_id)
{        
    	mass = getRandInt(STARBASE_MASS_MIN, STARBASE_MASS_MAX);
        docking_station = getNewDockingStation(race_id);
}

StarBase :: ~StarBase() 
{
	delete docking_station;
}    
    
DockingStation* StarBase :: getDockingStation() const { return docking_station; }
		
		
void StarBase :: update_inSpace(int time, bool show_effect)
{
...
void StarBase :: updateRenderStuff()
{
    	//points.update(); 
    	shield->update();
...
void StarBase :: render_inSpace() const
{   
        //if (ableTo.GRAB == true)
        //{
                //renderGrappleTrail();
...
void StarBase :: render_atPlanet() const
{
	//renderKorpus();
...
StarBase* getNewStarBase()
{
	...
	data_korpus.render_TURRELS = true; 
    	...
    	StarBase* starbase = new StarBase(RACE_0_ID);
	...
	starbase->setLifeData(data_life);
    	...
    	starbase->getDockingStation()->createInternals();
