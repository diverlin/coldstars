...
DockingStation* Planet :: getDockingStation() const { return docking_station; }
...
{      
	docking_station->update_inSpace_inStatic();
}
...
void Planet :: renderInfo_inSpace()
{  
	updateInfo();
...
Planet* getNewPlanet(int orbit_radius)
{
	...
	planet_data.scale         = getRandInt(PLANET_SIZE_MIN, PLANET_SIZE_MAX);  
        ...
        Planet* planet = new Planet(RACE_0_ID, population);
	...
	planet->setMesh(g_SPHERE_MESH);	
      	...
      	planet->getDockingStation()->createInternals();
