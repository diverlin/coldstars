...
m_Offset(0.0),
m_DeltaColor(0.0),
  m_DeltaBrightThreshold(0.0),
  m_OffsetSpeed(0.0002)
{ 
    SetId(id);
...
float Star::GetBrightThreshold() const { return GetTextureOb().GetData().brightThreshold + m_DeltaBrightThreshold; }
...
void Star::ChangeLightThreshold(float delta)
{
    m_DeltaBrightThreshold += delta;
}

void Star::ChangeOffseTextureSpeed(float delta)
{
    m_OffsetSpeed -= delta;
}
