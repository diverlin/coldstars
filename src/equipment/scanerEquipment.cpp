...
#include "scanerEquipment.h"

ScanerEquipment :: ScanerEquipment()
{}

ScanerEquipment :: ScanerEquipment(TextureOb* _pTo_itemTexOb, 
				   int _scan_orig, 
...
{
    	CommonForEquipment_init(SCANER_ID, _pTo_itemTexOb, _modules_num_max, _mass, _condition_max, _deterioration_rate);
...
void ScanerEquipment :: updateInfo()
{
...
std::string ScanerEquipment :: returnScanStr()
{
     	if (scan_add == 0)
...
ScanerEquipment* scanerEquipmentGenerator(int race_id, int revision_id)
{
    	...
    	TextureOb* pTo_itemTexOb = g_TEXTURE_MANAGER.returnPointerToRandomTexObFromList(&g_TEXTURE_MANAGER.ScanerEquipment_texOb_pList);   
    	...
    	ScanerEquipment* _scaner_equipment = new ScanerEquipment(pTo_itemTexOb, scan_orig, modules_num_max, mass, condition_max, deterioration_rate);
    	return _scaner_equipment;
}
