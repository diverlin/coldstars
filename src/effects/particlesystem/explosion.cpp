...
void  createExplosion(StarSystem* _starsystem, vec2f _center_pos, int obSize)
{
	...
	if (obSize < 4)
	{
		...
		data_particle.size_start  = 25 * obSize;                                 
                ...
                texOb_particle = g_TEXTURE_MANAGER.getTexObByColorId(TEXTURE::PARTICLE_EFFECT_ID, RED_COLOR_ID);
    	...
    	else    
	{
		...
		data_particle.size_start  = 25 * (obSize-1);
                ...
                texOb_particle = g_TEXTURE_MANAGER.getTexObByColorId(TEXTURE::PARTICLE_EFFECT_ID, YELLOW_COLOR_ID);
