...
//class ShopItem():
    //def __init__(self, item_icon_tex, (item_icon_w, item_icon_h), rect):
        //self.item_icon_tex, (self.item_icon_w, self.item_icon_h)       = item_icon_tex, (item_icon_w, item_icon_h)
        //self.icon_rect = rect
        //self.amount  = 0
        //self.price = 0
        //self.subtype = None

        //self.scroll_rect   = pygame.Rect(self.icon_rect[0], self.icon_rect[1], self.icon_rect[2], self.icon_rect[3])

    //def renderIcon(self):
        //drawTexturedRect(self.item_icon_tex, self.icon_rect, -1)

    //def renderAmount(self):
        //drawDynamicLabelList(slot_marked_tex, (self.icon_rect[0] + (self.icon_rect[2] + 5), self.icon_rect[1] + self.icon_rect[3]), [str(self.amount) + ', 1 = ' + str(self.price) + '$'])
...
//class Shop():
    //def __init__(self, bg_texOb):
        ...
        //self.medicine = ShopItem(medicine_icon_tex, (medicine_icon_w, medicine_icon_h), medicine_icon_rect )
        //self.item_list.append(self.medicine)

        //self.military = ShopItem(military_icon_tex, (military_icon_w, military_icon_h), military_icon_rect )
        ...
        //self.exclusive = ShopItem(exclusive_icon_tex, (exclusive_icon_w, exclusive_icon_h), exclusive_icon_rect )
    ...
    //def updateMineralsPrice(self):
        //self.minerals.price   = MINERAL_BASE_PRICE   + int(MINERAL_BASE_PRICE   * MINERALS_AMOUNT_MAX / (float(self.minerals.amount) + 1))

    //def updateFoodPrice(self):
        //self.food.price       = FOOD_BASE_PRICE      + int(FOOD_BASE_PRICE      * FOOD_AMOUNT_MAX     / (float(self.food.amount) + 1))

    //def updateMedicinePrice(self):
        //self.medicine.price   = MEDICINE_BASE_PRICE  + int(MEDICINE_BASE_PRICE  * MEDICINE_AMOUNT_MAX / (float(self.medicine.amount) + 1))

    //def updateMilitaryPrice(self):
        //self.military.price   = MILITARY_BASE_PRICE  + int(MILITARY_BASE_PRICE  * MILITARY_AMOUNT_MAX / (float(self.military.amount)  + 1))

    //def updateDrugPrice(self):
        //self.drug.price       = DRUG_BASE_PRICE      + int(DRUG_BASE_PRICE      * DRUG_AMOUNT_MAX     / (float(self.drug.amount) + 1))

    //def updateExclusivePrice(self):
        //self.exclusive.price  = EXCLUSIVE_BASE_PRICE + int(EXCLUSIVE_BASE_PRICE * EXCLUSIVE_AMOUNT_MAX / (float(self.exclusive.amount) + 1))
