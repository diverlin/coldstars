...
void MicroScenarioGrab::Enter(Npc* npc) const
{
    ...
    Logger::Instance().Log("npc_id="+ceti::int2str(npc->GetId())+" ENTER MicroScenarioGrab");
    #endif        
}
...
{
    #if AISCENARIO_LOG_ENABLED == 1 
    Logger::Instance().Log("npc_id="+ceti::int2str(npc->GetId())+" EXIT MicroScenarioGrab");
    #endif    
}
...
{
    return "MicroScenarioGrab ob_id = " + ceti::int2str(npc->GetStateMachine().GetMicroTaskManager().GetTarget()->GetId());
}
