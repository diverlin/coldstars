...
{    
    LOG("vehicle_id/npc_id="+std::to_string(npc->GetVehicle()->id())+"/"+std::to_string(npc->id())+"  ENTER MicroScenarioJump");
    
    npc->GetVehicle()->driveComplex().SetTarget(npc->GetStateMachine().GetMicroTaskManager().GetTarget()->starsystem(), NAVIGATOR_ACTION::KEEP_CLOSE_ID);
}
...
{
    LOG("vehicle_id/npc_id="+std::to_string(npc->GetVehicle()->id())+"/"+std::to_string(npc->id())+" EXIT MicroScenarioJump");
}

/* virtual */
std::string MicroScenarioJump::GetDescription(Npc* npc) const
{
    return "JUMP to ss_id = " + std::to_string( npc->GetStateMachine().GetMicroTaskManager().GetTarget()->id());
}
