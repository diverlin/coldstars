...
Angar::Angar(descriptor::AngarDescr* descr, model::Angar* model)
    :
      Room(descr, model)
...
bool Angar::chargeRocketEquipment(Npc* npc, Rocket* rocket_equipment) const
{
    assert(false);
...
{
    for (VehicleSlot* slot: m_vehicle_slots) {
        if (!slot->vehicle()) {
            return slot;
        }
    }
...
{
    VehicleSlot* slot = __slot(vehicle);
    if (slot) {
        if (slot->release()) {
