...
void Store::__add(slot::Item* item_slot)
{ 
    item_slot->setOwner(this);
    m_item_slots.push_back(item_slot);
}

void Store::__add(slot::Vehicle* vehicle_slot)
{ 
...
{
    slot::Vehicle* vehicle_slot = freeVehicleSlot();
    if (vehicle_slot) {
...
void Store::sellVehicle(Npc* npc, slot::Vehicle* vehicle_slot, int price)
{
    assert(false);
