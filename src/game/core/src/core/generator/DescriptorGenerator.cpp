...
glm::vec3 randPlanetoidDirection() {
    return glm::normalize(glm::vec3(meti::rand_float(0.1, 0.3), 1.0, -1.0));
}
...
genGalaxy(const std::vector<int_t>& sectors) {
    ...
    int num = meti::rand_int(1,3);
    for(int i=0; i<num; ++i) {
        descr->sectors.push_back(meti::rand_element(sectors));
    }
...
genSector(const std::vector<int_t>& starsystems) {
    ...
    int num = meti::rand_int(1,3);
    for(int i=0; i<num; ++i) {
        descr->starsystems.push_back(meti::rand_element(starsystems));
    }
    descriptor::Manager::get().add(descr);
    return descr;
}
...
genStar()
{
    ...
    int orbit_radius = meti::rand_int(descriptor::Star::DISTANCE_MIN,
                                        descriptor::Star::DISTANCE_MAX);
    descr->setRadiusA(orbit_radius);
    descr->setRadiusB(orbit_radius);
    descr->setOrbitPhi(0);
    float speed = meti::rand_int(descriptor::Star::SPEED_MIN,
                                   descriptor::Star::SPEED_MAX) / float(orbit_radius);
    descr->setSpeed(speed);
    descr->setClockwise(meti::rand_bool());

    float size = meti::rand_int(descriptor::Star::SCALE_MIN,
...
genPlanet()
{
    ...
    int orbit_radius = meti::rand_int(descriptor::Planet::DISTANCE_MIN,
                                        descriptor::Planet::DISTANCE_MAX);
    descr->setRadiusA(orbit_radius);
    descr->setRadiusB(orbit_radius);
    descr->setOrbitPhi(0);
    float speed = meti::rand_int(descriptor::Planet::SPEED_MIN,
                                   descriptor::Planet::SPEED_MAX) / float(orbit_radius);
    descr->setSpeed(speed);
    descr->setClockwise(meti::rand_bool());

    float size = meti::rand_int(descriptor::Planet::SCALE_MIN,
...
genAsteroid()
{
    ...
    descr->setArmor(meti::rand_int(descriptor::Asteroid::ARMOR_MIN,
                                    descriptor::Asteroid::ARMOR_MAX));

    int orbit_radiusA = meti::rand_int(descriptor::Asteroid::DISTANCE_MIN,
                                        descriptor::Asteroid::DISTANCE_MAX);
    int orbit_radiusB = meti::rand_int(descriptor::Asteroid::DISTANCE_MIN,
                                        descriptor::Asteroid::DISTANCE_MAX);
    descr->setRadiusA(orbit_radiusA);
    descr->setRadiusB(orbit_radiusB);
    descr->setOrbitPhi(0);
    float speed = meti::rand_int(descriptor::Asteroid::SPEED_MIN,
                                   descriptor::Asteroid::SPEED_MAX) / float((orbit_radiusA+orbit_radiusB)/2.0);
    descr->setSpeed(speed);
    descr->setClockwise(meti::rand_bool());

    float size = meti::rand_int(descriptor::Asteroid::SCALE_MIN,
...
genShip()
{
    ...
    int space       = size_id*100 + meti::rand_int(100);
    int armor       = space;
    int protection  = protection_rate*meti::rand_int(size_id);
    int temperature = 100;
    int price       = meti::rand_int(200, 400)*size_id;
    ...
    int slot_artefact_num = meti::rand_int(1, SLOT_ARTEFACT_TYPES.size());
    int slot_cargo_num    = meti::rand_int(SLOT_CARGO_TYPES.size()/2, SLOT_CARGO_TYPES.size()) * otsec_rate;

    descriptor::Ship* descr = new descriptor::Ship;
    descr->setRace(race_id);
    for(int i=0; i<slot_weapon_num; ++i) {
        float length = meti::rand_float(-0.8f, 0.8f);
...
genBak(int race, int tech_level)
{
    ...
    int modules       = meti::rand_int(Bak::MODULES_NUM_MIN, Bak::MODULES_NUM_MAX);
    int mass          = meti::rand_int(Bak::MASS_MIN, Bak::MASS_MAX);
    int condition     = meti::rand_int(Bak::CONDITION_MIN, Bak::CONDITION_MAX);
    int deterioration = 1;

    int fuel = meti::rand_int(Bak::FUEL_MIN, Bak::FUEL_MAX) * (1 + int(Bak::FUEL_TECH_RATE * tech_level));
...
genDrive(int race, int tech_level)
{
    ...
    int modules       = meti::rand_int(Drive::MODULES_NUM_MIN, Drive::MODULES_NUM_MAX);
    int mass          = meti::rand_int(Drive::MASS_MIN, Drive::MASS_MAX);
    int condition     = meti::rand_int(Drive::CONDITION_MIN, Drive::CONDITION_MAX);
    int deterioration = 1;

    int speed = meti::rand_int(Drive::SPEED_MIN, Drive::SPEED_MAX) * (1 + int(Drive::SPEED_TECH_RATE * tech_level));
    int hyper = meti::rand_int(Drive::HYPER_MIN, Drive::HYPER_MAX) * (1 + int(Drive::HYPER_TECH_RATE * tech_level));
    //jeti::Mesh* mesh = MeshCollector::Instance().getMesh(mesh::type::PLANE);
...
genDroid(int race, int tech_level)
{
    ...
    int modules       = meti::rand_int(Droid::MODULES_NUM_MIN, Droid::MODULES_NUM_MAX);
    int mass          = meti::rand_int(Droid::MASS_MIN,        Droid::MASS_MAX);
    int condition     = meti::rand_int(Droid::CONDITION_MIN,   Droid::CONDITION_MAX);
    int deterioration = 1;

    int repair = meti::rand_int(Droid::REPAIR_MIN, Droid::REPAIR_MAX) * (1 + int(Droid::REPAIR_TECH_RATE * tech_level));
    //jeti::Mesh* mesh = MeshCollector::Instance().getMesh(mesh::type::PLANE);
...
genGrapple(int race, int tech_level)
{
    ...
    int modules = meti::rand_int(Grapple::MODULES_NUM_MIN, Grapple::MODULES_NUM_MAX);
    int mass = meti::rand_int(Grapple::MASS_MIN,        Grapple::MASS_MAX);
    int condition = meti::rand_int(Grapple::CONDITION_MIN,   Grapple::CONDITION_MAX);
    int deterioration = 1;

    //jeti::Mesh* mesh = MeshCollector::Instance().getMesh(mesh::type::PLANE);
    //jeti::control::TextureOb* texOb_item = TextureCollector::Instance().getTextureByTypeId(TYPE::TEXTURE::GRAPPLE_EQUIPMENT);

    int strength   = meti::rand_int(Grapple::STRENGTH_MIN, Grapple::STRENGTH_MAX) * (1 + int(Grapple::STRENGTH_TECH_RATE * tech_level));
    int radius     = meti::rand_int(Grapple::RADIUS_MIN,   Grapple::RADIUS_MAX)   * (1 + int(Grapple::RADIUS_TECH_RATE * tech_level));
    int speed      = meti::rand_int(Grapple::SPEED_MIN,    Grapple::SPEED_MAX)    * (1 + int(Grapple::SPEED_TECH_RATE * tech_level));
...
genLazer(int race, int tech_level)
{
    ...
    int modules       = meti::rand_int(Lazer::MODULES_NUM_MIN, Lazer::MODULES_NUM_MAX);
    int mass          = meti::rand_int(Lazer::MASS_MIN,        Lazer::MASS_MAX);
    int condition     = meti::rand_int(Lazer::CONDITION_MIN,   Lazer::CONDITION_MAX);
    int deterioration = 1;

    int damage = meti::rand_int(Lazer::DAMAGE_MIN, Lazer::DAMAGE_MAX) * (1 + int(Lazer::DAMAGE_TECH_RATE * tech_level));
    int radius = meti::rand_int(Lazer::RADIUS_MIN, Lazer::RADIUS_MAX) * (1 + int(Lazer::RADIUS_TECH_RATE * tech_level));
    //jeti::Mesh* mesh = MeshCollector::Instance().getMesh(mesh::type::PLANE);
    //jeti::control::TextureOb* texOb_item = TextureCollector::Instance().getTextureByTypeId(TYPE::TEXTURE::DROID_EQUIPMENT);
...
genProtector(int race, int tech_level)
{
    ...
    int modules = meti::rand_int(Protector::MODULES_NUM_MIN, Protector::MODULES_NUM_MAX);
    int mass    = meti::rand_int(Protector::MASS_MIN,        Protector::MASS_MAX);
    int condition = meti::rand_int(Protector::CONDITION_MIN, Protector::CONDITION_MAX);
    int deterioration = 1;

    //jeti::Mesh* mesh = MeshCollector::Instance().getMesh(mesh::type::PLANE);
    //jeti::control::TextureOb* texOb_item = TextureCollector::Instance().getTextureByTypeId(TYPE::TEXTURE::PROTECTOR_EQUIPMENT);

    int protection = meti::rand_int(Protector::PROTECTION_MIN, Protector::PROTECTION_MAX);
...
genRadar(int race, int tech_level)
{
    ...
    int modules = meti::rand_int(Radar::MODULES_NUM_MIN, Radar::MODULES_NUM_MAX);
    int mass    = meti::rand_int(Radar::MASS_MIN,        Radar::MASS_MAX);
    int condition = meti::rand_int(Radar::CONDITION_MIN, Radar::CONDITION_MAX);
    int deterioration = 1;

    //jeti::Mesh* mesh = MeshCollector::Instance().getMesh(mesh::type::PLANE);
    //jeti::control::TextureOb* texOb_item = TextureCollector::Instance().getTextureByTypeId(TYPE::TEXTURE::RADAR_EQUIPMENT);

    int radius = meti::rand_int(Radar::RADIUS_MIN, Radar::RADIUS_MAX);
...
genRocket(int race, int tech_level)
{
    ...
    int modules = meti::rand_int(Rocket::MODULES_NUM_MIN, Rocket::MODULES_NUM_MAX);
    int mass    = meti::rand_int(Rocket::MASS_MIN,        Rocket::MASS_MAX);
    int condition = meti::rand_int(Rocket::CONDITION_MIN, Rocket::CONDITION_MAX);
    int deterioration = 1;

    //jeti::Mesh* mesh = MeshCollector::Instance().getMesh(mesh::type::PLANE);
    //jeti::control::TextureOb* texOb_item = TextureCollector::Instance().getTextureByTypeId(TYPE::TEXTURE::RADAR_EQUIPMENT);

    int ammo = meti::rand_int(Rocket::AMMO_MIN, Rocket::AMMO_MAX);
    int damage = meti::rand_int(Rocket::DAMAGE_MIN, Rocket::DAMAGE_MAX);
    int radius = meti::rand_int(Rocket::RADIUS_MIN, Rocket::RADIUS_MAX);
...
genScaner(int race, int tech_level)
{
    ...
    int modules = meti::rand_int(Scaner::MODULES_NUM_MIN, Scaner::MODULES_NUM_MAX);
    int mass = meti::rand_int(Scaner::MASS_MIN,        Scaner::MASS_MAX);
    int condition = meti::rand_int(Scaner::CONDITION_MIN,   Scaner::CONDITION_MAX);
    int deterioration = 1;

    //jeti::Mesh* mesh = MeshCollector::Instance().getMesh(mesh::type::PLANE);
    //jeti::control::TextureOb* texOb_item = TextureCollector::Instance().getTextureByTypeId(TYPE::TEXTURE::SCANER_EQUIPMENT);

    int scan = meti::rand_int(Scaner::SCAN_MIN, Scaner::SCAN_MAX) * (1 + int(Scaner::SCAN_TECH_RATE * tech_level));
