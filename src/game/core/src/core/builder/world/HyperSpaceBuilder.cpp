...
HyperSpace::gen()
{
    ...
    if (!descriptor::Manager::get().hasType(descriptor::Type::HYPERSPACE)) {
        descr = descriptor::genHyperSpace();
    } else {
        descr = descriptor::Manager::get().randHyperSpace();
    }
    assert(descr);
    return gen(descr);
}
...
{
    descriptor::HyperSpace* descr = descriptor::Manager::get().hyperSpace(descriptor_id);
...
HyperSpace::__genTemplate(descriptor::HyperSpace* descr, int_t ob_id)
{
    ...
    manager::EntityManager::get().setHyperSpace(hyperspace);
    return hyperspace;
}
