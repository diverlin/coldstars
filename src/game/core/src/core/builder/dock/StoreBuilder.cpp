...
StoreBuilder::gen()
{
    core::StoreDescr* descr = nullptr;
    if (!core::shortcuts::descriptors()->hasType(core::Type::STORE)) {
        descr = core::genStore();
    } else {
...
core::control::Store*
StoreBuilder::gen(core::StoreDescr* descr)
{
...
{
    StoreModel* model = new StoreModel(descr->id());
    assert(model);
