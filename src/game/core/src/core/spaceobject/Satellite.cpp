...
Satellite::Satellite(descriptor::SatelliteDescr* descr, model::Satellite* model)
    :
      Vehicle(descr, model)
...
void Satellite::BindParent(const SpaceObject* const parent)
{
    assert(false);
    //setParent(parent);
    m_Orbit.calcPath(2*1.1*parent->collisionRadius(), 1.0, meti::rand::gen_bool());
}
...
//    setSubTypeId(entity::Type::SATELLITE);
