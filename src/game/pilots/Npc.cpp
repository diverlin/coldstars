...
void Npc::AddExpirience(int expirience, bool show_effect)
{
	...
	if (show_effect == true)
	{
       		VerticalFlowText* text = new VerticalFlowText(int2str(expirience), 12, vehicle->GetPoints().GetCenterXY(), COLOR::COLOR4I_BLUE_LIGHT, 10);
...
{
     	return GetStarSystem()->GetClosestInhabitedPlanet(vehicle->GetPoints().GetCenterXY());  // improove
}
