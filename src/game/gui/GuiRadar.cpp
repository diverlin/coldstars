...
void GuiRadar::Render() const
{
	...
	range_rect.SetCenter(rect.GetCenter() + player->GetNpc()->GetVehicle()->GetPoints().GetCenterXY() * scale);
	...
	enable_POINTSPRITE();       	
		for (unsigned int i=0; i<entity_vec.size(); i++)
		{
			...
			drawParticle(size, rect.GetCenter() + entity_vec[i]->GetPoints().GetCenterXY()*scale);			
		}
	disable_POINTSPRITE(); 
}
