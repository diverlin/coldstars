...
class Camera():
      ...
      def direction(self, (vpCoordinate_x, vpCoordinate_y), player):
          if ((player.rect.centerx <  VIEW_WIDTH/8 + vpCoordinate_x) or (player.rect.centery < VIEW_HEIGHT/8 + vpCoordinate_y) or (player.rect.centerx > VIEW_WIDTH/1.2 + vpCoordinate_x) or (player.rect.centery > VIEW_HEIGHT/1.2 + vpCoordinate_y)) and self.list_END == True:
             self.camera_target_x = player.rect.centerx - VIEW_WIDTH / 2
             self.camera_target_y = player.rect.centery - VIEW_HEIGHT / 2
             self.camera_direction_list_x, self.camera_direction_list_y, not_relevant = follow_static_obj((vpCoordinate_x, vpCoordinate_y), (self.camera_target_x, self.camera_target_y), self.speedr)
      ...
      def update(self, (vpCoordinate_x, vpCoordinate_y)):
          if self.list_END == False:
             if vpCoordinate_x != self.camera_target_x or vpCoordinate_y != self.camera_target_y:
