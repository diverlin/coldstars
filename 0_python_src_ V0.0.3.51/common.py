...
class CommonInstance():
    ...
    def updateAnimationFrame(self):
        ...
        if t - self._last_update > self._delay:
           ...
           if self._tex_frame == (self.len_texture_ID - 1):
              self.frames_ENDED = True  # this flag needs for not loop animation such as explosions
           self._last_update = t
    ...
    def renderDynamicFrame(self):
        drawDynamic(self.texture_ID, (self.rect.centerx, self.rect.centery), self.angle, (self.minus_w_div_2, self.minus_h_div_2,  self.plus_w_div_2, self.plus_h_div_2 ))
    ...
    def renderDynamicFrames(self):
        drawDynamic(self.texture_ID[self._tex_frame], (self.rect.centerx, self.rect.centery), self.angle, (self.minus_w_div_2, self.minus_h_div_2, self.plus_w_div_2, self.plus_h_div_2 ))
...
# returns the x,y lists for planet, asteroid orbit. This algorythm is weight, that is why it must be used once, for static orbit calculation. Use get_orbit_coord_low_precision for better merfomance
    ...
    if radius_A != radius_B:
       for alfa in alfa_LIST:
           x_coord = x_0 + radius_A * cos(alfa) * cos(fi) - radius_B * sin(alfa) * sin(fi)
           y_coord = y_0 + radius_A * cos(alfa) * sin(fi) + radius_B * sin(alfa) * cos(fi)
...
def returnFirstFreeSlotBySlotType(slot_type, slot_subtype, slot_list):
    ...
    for slot in slot_list:
        if slot.item == None and slot.type == slot_type and slot.subtype == slot_subtype:
           return slot
    return None
