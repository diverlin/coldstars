...
from GLSL_stuff import returnShockWavesDataInArrays, ShockWaveEffect
...
class starSystemInstance():
      ...
      def starImpact(self):
          ...
          for k in self.SHIP_list:
              dist = lengthBetweenPoints((xc, yc), (k.points.center[0], k.points.center[1]))
      ...
      def bombExplosionCollision(self, bomb):
          ...
          for a in self.ASTEROID_list:
              dist = lengthBetweenPoints((xc, yc), (a.points.center[0], a.points.center[1]))
              dist_rate = float(bomb.radius)/(dist+0.00001)
              if dist_rate > 1:
                 a.hit(bomb, bomb)
          ...
          for b in self.BOMB_list:
              #if b.ID != bomb.ID:
                 dist = lengthBetweenPoints((xc, yc), (b.points.center[0], b.points.center[1]))
                 dist_rate = float(bomb.radius)/(dist+0.00001)
                 if dist_rate > 1:
                    b.hit(bomb, bomb)
      ...
      def blackholeAutoGenerator(self, timer, max_num):
          if timer < 0:
             if len(self.BLACKHOLE_list) < max_num:
                ...
                bh = BlackHoleInstance('blackhole', texOb, self, randint(-2000, 2000), randint(-2000, 2000))
      ...
      def mineralAutoGenerator(self, timer, max_num):
          ...
          while len(self.MINERAL_list) < max_num:
                ...
                m = MineralInstance(texOb, self, randint(100, 300), randint(100, 300), randint(100, 300), randint(100, 300))
      ...
      def addNumMinerals(self, (x, y), num):
          ...
          while i < num:
                ...
                texOb = returnRandomFromTheList(TEXTURE_MANAGER.mineral_texOb_list)
                self.MINERAL_list.append(MineralInstance(texOb, self, x, y, target_x, target_y))
                alpha += d_alpha
                i += 1
      ...
      def addContainers(self, (x, y), item_drop_queue):   #  move to ship class
          ...
          while i < len(item_drop_queue):
                ...
                self.CONTAINER_list.append(Container(texOb, item_drop_queue[i], self, x, y, target_x, target_y))
                alpha += d_alpha
                i += 1
      ...
      def asteroidCollision(self):
          ...
          for a in self.ASTEROID_list:
              for k in self.SHIP_list:
                  if collisionBetweenCenters(a.points.center, k.points.center, k.collision_threshold):
                     k.hit(a, a)
                     a.hit(a, a)
                     collide = True
                     break
              ...
              if collide == False:
                 for s in self.STAR_list:
                     if collisionBetweenCenters(a.points.center, s.points.center, s.collision_threshold):
                        a.hit(a, a)
                        collide = True
                        break
              ...
              if collide == False:
                 for p in self.PLANET_list:
                     if collisionBetweenCenters(a.points.center, p.points.center, p.collision_threshold):
                        a.hit(a, a)
                        collide = True
                        break
      ...
      def rocketCollision(self):
          ...
          for r in self.ROCKET_list:
              for k in self.SHIP_list:
                  if r.owner.ID != k.ID:
                     if collisionBetweenCenters(r.points.center, k.points.center, k.collision_threshold):
                        k.hit(r.owner, r)
                        r.hit(r.owner, r)
                        collide = True
                        break
              ...
              if collide == False:
                 for a in self.ASTEROID_list:
                     if collisionBetweenCenters(r.points.center, a.points.center, a.collision_threshold):
                        a.hit(r.owner, r)
                        r.hit(r.owner, r)
                        collide = True
                        break
              ...
              if collide == False:
                 for m in self.MINERAL_list:
                     if collisionBetweenCenters(r.points.center, m.points.center, m.collision_threshold):
                        m.hit(r.owner, r)
                        r.hit(r.owner, r)
                        collide = True
                        break
              ...
              if collide == False:
                 for c in self.CONTAINER_list:
                     if collisionBetweenCenters(r.points.center, c.points.center, c.collision_threshold):
                        c.hit(r.owner, r)
                        r.hit(r.owner, r)
                        collide = True
                        break
      ...
      def mouseInteraction(self, timer, (mx, my), (vpCoordinate_x, vpCoordinate_y), (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED), show_RADAR, grapple_SELECTED, (lb, rb)):
          ...
          if CURSOR_INTERSECT_OBJECT != True:
             for k in self.visible_SHIP_list:
                 ship_cursor_dist = lengthBetweenPoints((k.points.center[0], k.points.center[1]), (mxvp, myvp))
                 if ship_cursor_dist < player.points.w/4.0:
                    ...
                    if lb == True:
                       player.setWeaponsToTarget(k, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED))
         ...
         ####################################################################################################
          ...
          if CURSOR_INTERSECT_OBJECT != True:
             for sp in self.visible_SPUTNIK_list:
                 sputnik_cursor_dist = lengthBetweenPoints((sp.points.center[0], sp.points.center[1]), (mxvp, myvp))
                 if sputnik_cursor_dist < sp.w/4:
                    CURSOR_INTERSECT_OBJECT = True
                    sp.renderInfo()

                    if lb == True:
                       player.setWeaponsToTarget(sp, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED))
          ####################################################################################################
          ...
          if CURSOR_INTERSECT_OBJECT != True:
             for a in self.visible_ASTEROID_list:
                 asteroid_cursor_dist = lengthBetweenPoints((a.points.center[0], a.points.center[1]), (mxvp, myvp))
                 if asteroid_cursor_dist < a.w/3:
                    ...
                    if lb == True:
                       player.setWeaponsToTarget(a, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED))
          ####################################################################################################

          ####################################################################################################
          if CURSOR_INTERSECT_OBJECT != True:
             for p in self.visible_PLANET_list:
                 planet_cursor_dist = lengthBetweenPoints((p.points.center[0], p.points.center[1]), (mxvp, myvp))
                 if planet_cursor_dist < p.w/4:
                    ...
                    if lb == True:
                       ...
                       player.setTargetPosCoord((player.target_planet.points.center[0], player.target_planet.points.center[1]))
          ...
          if CURSOR_INTERSECT_OBJECT != True:
             for c in self.visible_CONTAINER_list:
                 container_cursor_dist =  lengthBetweenPoints((c.points.center[0], c.points.center[1]), (mxvp, myvp))
                 if container_cursor_dist < c.w/1.5:
                    ...
                    if lb == True:
                       if grapple_SELECTED == True:
                          ...
                          player.setWeaponsToTarget(c, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED))


          ####################################################################################################
          if CURSOR_INTERSECT_OBJECT != True:
             for b in self.visible_BOMB_list:
                 bomb_cursor_dist = lengthBetweenPoints((b.points.center[0], b.points.center[1]), (mxvp, myvp))
                 if bomb_cursor_dist < b.w/1.5:
                    ...
                    if lb == True:
                       if grapple_SELECTED == True:
                          ...
                          player.setWeaponsToTarget(b, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED))

          ####################################################################################################
          if CURSOR_INTERSECT_OBJECT != True:
             for m in self.visible_MINERAL_list:
                 mineral_cursor_dist = lengthBetweenPoints((m.points.center[0], m.points.center[1]), (mxvp, myvp))
                 if mineral_cursor_dist < m.w/2:
                    ...
                    if lb == True:
                       if grapple_SELECTED == True:
                          ...
                          player.setWeaponsToTarget(m, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED))
          ###################################################################################################
          ...
          if CURSOR_INTERSECT_OBJECT != True:
             for b in self.visible_BLACKHOLE_list:
                 blackhole_cursor_dist = lengthBetweenPoints((b.points.center[0], b.points.center[1]), (mxvp, myvp))
          ...
          if CURSOR_INTERSECT_OBJECT != True:
             for s in self.visible_STAR_list:
                 star_cursor_dist = lengthBetweenPoints((s.points.center[0], s.points.center[1]), (mxvp, myvp))
      ...
      def findVisibleEntities(self, (vpCoordinate_x, vpCoordinate_y)):
          ...
          self.visible_STAR_list      = [s for s in self.STAR_list      if isObjectVisible((s.points.center[0], s.points.center[1]), (s.w, s.h), (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord))]
          self.visible_PLANET_list    = [p for p in self.PLANET_list    if isObjectVisible((p.points.center[0], p.points.center[1]), (p.w, p.h), (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord))]
          ...
          self.visible_ASTEROID_list  = [a for a in self.ASTEROID_list  if isObjectVisible((a.points.center[0], a.points.center[1]), (a.w, a.h), (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord))]
          self.visible_MINERAL_list   = [m for m in self.MINERAL_list   if isObjectVisible((m.points.center[0], m.points.center[1]), (m.w, m.h), (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord))]
          self.visible_CONTAINER_list = [c for c in self.CONTAINER_list if isObjectVisible((c.points.center[0], c.points.center[1]), (c.w, c.h), (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord))]
          ...
          self.visible_SHIP_list      = [k for k in self.SHIP_list      if isObjectVisible((k.points.center[0], k.points.center[1]), (k.w, k.h), (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord))]
          self.visible_ROCKET_list    = [r for r in self.ROCKET_list    if isObjectVisible((r.points.center[0], r.points.center[1]), (r.w, r.h), (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord))]
          ...
          self.visible_effect_LAZER_list    = [e for e in self.effect_LAZER_list    if isLineVisible(e.p1, e.p2, (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord))]
          

      def radarEntitiesFilter(self):  # TRY to MERGE with find visible entities
          distance = player.radar.radius

          self.visible_SHIP_list      = [ k for k in self.visible_SHIP_list      if collisionBetweenCenters( k.points.center, player.points.center, distance ) ]
          self.visible_SPUTNIK_list   = [ s for s in self.visible_SPUTNIK_list   if collisionBetweenCenters( s.points.center, player.points.center, distance ) ]

          self.visible_MINERAL_list   = [ m for m in self.visible_MINERAL_list   if collisionBetweenCenters( m.points.center, player.points.center, distance ) ]
          self.visible_CONTAINER_list = [ c for c in self.visible_CONTAINER_list if collisionBetweenCenters( c.points.center, player.points.center, distance ) ]
          self.visible_BOMB_list      = [ b for b in self.visible_BOMB_list      if collisionBetweenCenters( b.points.center, player.points.center, distance ) ]

          self.visible_effect_EXPLOSION_list = [ e for e in self.visible_effect_EXPLOSION_list if collisionBetweenCenters(e.center, player.points.center, distance) ]
          self.visible_effect_DAMAGE_list    = [ e for e in self.visible_effect_DAMAGE_list    if collisionBetweenCenters(e.center, player.points.center, distance) ]

          self.visible_effect_LAZER_list = [ e for e in self.visible_effect_LAZER_list if collisionBetweenCenters([e.ex, e.ey], player.points.center, distance) ]   # ????!!!!



         

      def render_TEST(self, fbo0, fbo1, fbo2, bloom, program_ShockWave, program_Shafts, vpCoordinate_x, vpCoordinate_y, lb = False, rb = False):
          ...
          for ds in self.distant_stars_list:
              ...
              glBegin(GL_POINTS)
              glVertex3f(ds.pos_x - vx*ds.distance_rate, ds.pos_y - vy*ds.distance_rate, -2)
              glEnd()
          ...
          drawQuad(player.points.bottomLeft, player.points.bottomRight, player.points.topRight, player.points.topLeft, -2)
          ...
          for p in player.drive_jet.particles_list:
              p.update(player.drive_jet.velocity_x, player.drive_jet.velocity_y)   # matematika, update pozocii kazhdoj chasticy
      ...
      def renderEntities_NEW(self, fbo0, fbo1, fbo2, bloom, program_ShockWave, program_Shafts, vpCoordinate_x, vpCoordinate_y, lb = False, rb = False):
          ...
          glUniform4f(glGetUniformLocation(program_Shafts, 'sun_pos'), -float(vpCoordinate_x)/w, -float(vpCoordinate_y)/h, -100, 1)
          ...
          center_array, xyz_array, time_array = returnShockWavesDataInArrays(self.effect_SHOCKWAVE_list, vpCoordinate_x, vpCoordinate_y)
          #if randint(1,20) == 1:
          ...
          glUniform1i (glGetUniformLocation(program_ShockWave, 'imax'),        len_effect_SHOCKWAVE_list)
          glUniform2fv(glGetUniformLocation(program_ShockWave, 'center'),      len_effect_SHOCKWAVE_list, center_array)
          glUniform3fv(glGetUniformLocation(program_ShockWave, 'shockParams'), len_effect_SHOCKWAVE_list, xyz_array)
          glUniform1fv(glGetUniformLocation(program_ShockWave, 'time'),        len_effect_SHOCKWAVE_list, time_array)
      ...
      def removeDeadEntities(self): # can be moved to low freq exec
          ...
          for k in self.SHIP_remove_queue:
              ...
              print k.name, 'removed from world'
              del k
          self.SHIP_remove_queue = []
      ...
      def renderEntities_OLD(self, fbo0, fbo1, fbo2, bloom, program_ShockWave, program_Shafts, vpCoordinate_x, vpCoordinate_y, lb = False, rb = False):
          ######### EFFECT DEMONSTRATION
          if lb == True:
             for x in xrange(0, 2):
                 addExplosion(player)
          ...
          glLightfv(GL_LIGHT0, GL_POSITION,  (-vpCoordinate_x, -vpCoordinate_y, Z_POS_PLANET + 400, 1.0))
          ...
          glLightfv(GL_LIGHT0, GL_POSITION,  (-vpCoordinate_x, -vpCoordinate_y, Z_POS_PLANET, 1.0))
      ...
      def setBgColor(self):
          glColor4f(self.red, self.green, self.blue, 1.0) # restore BG palette after text render has changed it
      ...
      def renderRanges(self, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED), show_RADAR, grapple_SELECTED):
          if player.alive == True:
             if player.Gl_LIST_direction_ID != None:
                player.renderDirection()
             player.renderRadius((slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED), grapple_SELECTED, show_RADAR)
      ...
      def returnClosestCapturedSs(self):
          ...
          dist_min = lengthBetweenPoints((self.rectOnMap.centerx, self.rectOnMap.centery), (ss.rectOnMap.centerx, ss.rectOnMap.centery))

          for captured_ss in global_STARSYSTEM_CAPTURED_list:
              dist = lengthBetweenPoints((self.rectOnMap.centerx, self.rectOnMap.centery), (captured_ss.rectOnMap.centerx, captured_ss.rectOnMap.centery))
              if dist < dist_min:
                 dist_min = dist
                 ss = captured_ss
          return ss
...
def isObjectVisible((ob_centerx, ob_centery),  (ob_w, ob_h), (vpCoordinate_x, vpCoordinate_y), (x_startViewCoord, y_startViewCoord)):    # NEW!!!
