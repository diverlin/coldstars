...
class worldGenerator():
      ...
      def sputnikGenerator(self, p):
          (sp_tex, (sp_w, sp_h)) = SATELLITE_DATA_TUPLES[randint(0, len(SATELLITE_DATA_TUPLES) - 1)]
      ...
      def generateEntireStarsystem(self):
          ...
          self.generateNumPlanets(starsystem, randint(PLANET_PER_SYSTEM_MIN, PLANET_PER_SYSTEM_MAX))

          if starsystem.CAPTURED == False:
             self.generateNumFriendlyNPC(randint(SHIP_PER_SYSTEM_MIN, SHIP_PER_SYSTEM_MAX), starsystem)
          else:
             self.generateNumEnemyNPC(randint(ENEMY_SHIP_PER_SYSTEM_MIN, ENEMY_SHIP_PER_SYSTEM_MAX), starsystem)
      ...
      def generateStarSystem(self):
          ...
          starTexOb = TEXTURE_MANAGER.star_texOb_list[randint(0, len(TEXTURE_MANAGER.star_texOb_list) - 1)]
          ...
          starsystem.rectOnMap = pygame.Rect((randint(1.5*MAPOFFSETX, VIEW_WIDTH - 1.5*MAPOFFSETX), randint(1.5*MAPOFFSETY, VIEW_HEIGHT - 1.5*MAPOFFSETY)), (star.w / 30, star.h / 30))
      ...
      def generateBackground(self, starsystem, num_big, num_mid, num_rotated, num_dist_stars):
          # CREATE BACKGROUND
          
          starsystem.ds = distantStar(TEXTURE_MANAGER.distStar_texOb_list[randint(0, len(TEXTURE_MANAGER.distStar_texOb_list)-1)], randint(0, 1000), randint(0, 1000), randint(5, 30))  # needs for bug fixing with point sprite (HACK)
          ...
          i = 0
          while i < num_big:
              texOb = TEXTURE_MANAGER.nebulaStaticBig_texOb_list[randint(0, len(TEXTURE_MANAGER.nebulaStaticBig_texOb_list)-1)]
          ...
          i = 0
          while i < num_mid:
              texOb = TEXTURE_MANAGER.nebulaStaticMid_texOb_list[randint(0, len(TEXTURE_MANAGER.nebulaStaticMid_texOb_list)-1)]
          ...
          i = 0
          while i < num_rotated:
              texOb = TEXTURE_MANAGER.nebulaRotatedMid_texOb_list[randint(0, len(TEXTURE_MANAGER.nebulaRotatedMid_texOb_list)-1)]
          ...
          while i < num_dist_stars:
              ds = distantStar(TEXTURE_MANAGER.distStar_texOb_list[randint(0, len(TEXTURE_MANAGER.distStar_texOb_list)-1)], randint(0, 1000), randint(0, 1000), randint(5, 30))
      ...
      def generateNumPlanets(self, starsystem, planet_per_system):
          ...
          while planet_num < planet_per_system:
                ...
                selected_planet_class = allowed_planet_class_list[randint(0, len(allowed_planet_class_list) - 1)]
                ...
                planetTexOb = planet_allowed_TexOb_DATA[randint(0, len(planet_allowed_TexOb_DATA) - 1)]
               
                if planet_num == 0:
                   radius_A = randint(2 * PLANET_DISTANCE_MIN, 2 * PLANET_DISTANCE_MAX)
                else:
                   radius_A = start_point + randint(PLANET_DISTANCE_MIN, PLANET_DISTANCE_MAX)
                start_point = radius_A

                radius_B = radius_A
                                                
                p = PlanetInstance('p'+ str(self.planet_counter), SPHERE_MODEL, planetTexOb, program_light, race_id, population, type, starsystem, radius_A, radius_B, port, land)
      ...
      def generateNumEnemyNPC(self, num, starsystem):
          ...
          while ship_counter < num:
                ...
                k.item_list = [rocketGenerator(race_id), lazerGenerator(race_id), rocketGenerator(race_id), lazerGenerator(race_id), lazerGenerator(race_id), energyBlockGenerator(race_id), freezerGenerator(race_id), radarGenerator(race_id), grappleGenerator(race_id), driveGenerator(race_id), protectorGenerator(race_id), bakGenerator(race_id), droidGenerator(race_id), scanerGenerator(race_id)]
...
def generateNPC(race_id, subtype, subsubtype, starsystem, ship_name_num):
    ...
    if subtype == RANGER_ID and subsubtype == -1:
       subsubtype = TEXTURE_MANAGER.RACE0_SHIP_SUBTYPE_list[randint(0, len(TEXTURE_MANAGER.RACE0_SHIP_SUBTYPE_list) - 1)] # improve
    ...
    korpus = korpusGenerator(k.race, k.subtype, -1, -1) # - 1 means any
    k.item_list = [rocketGenerator(race_id), lazerGenerator(race_id), rocketGenerator(race_id), lazerGenerator(race_id), lazerGenerator(race_id), energyBlockGenerator(race_id), freezerGenerator(race_id), grappleGenerator(race_id), radarGenerator(race_id), driveGenerator(race_id), protectorGenerator(race_id), bakGenerator(race_id), droidGenerator(race_id), scanerGenerator(race_id)]
    k.setKorpus(korpus)
    k.initPositionInSpace()
