...
from render import drawFullScreenQuad, drawFullScreenTexturedQuad, drawFullScreenTexturedQuadBlurred
...
class bloomEffect():
      ...
      def pass0(self, tex, w, h):
          ...
          glUniform1f(glGetUniformLocation(self.program_ExtractBrightAreas, 'threshold'), self.brightThreshold)
      ...
      def pass1(self, w, h):
          ...
          drawFullScreenTexturedQuadBlurred(self.fbo1_pass0.texture, w, h, -999.0, self.program_blur)
          self.fbo1_pass1.deactivate()

          # RENDER TO FBO2
          self.fbo2_pass1.activate()
          drawFullScreenTexturedQuadBlurred(self.fbo2_pass0.texture, w/2, h/2, -999.0, self.program_blur)
          self.fbo2_pass1.deactivate()

          # RENDER TO FBO3
          self.fbo3_pass1.activate()
          drawFullScreenTexturedQuadBlurred(self.fbo3_pass0.texture, w/4, h/4, -999.0, self.program_blur)
          self.fbo3_pass1.deactivate()

          # RENDER TO FBO4
          self.fbo4_pass1.activate()
          drawFullScreenTexturedQuadBlurred(self.fbo4_pass0.texture, w/8, h/8, -999.0, self.program_blur)
          self.fbo4_pass1.deactivate()

      def pass2(self, w, h):
          # RENDER TO FBO1
          self.fbo1_pass2.activate()
          drawFullScreenTexturedQuadBlurred(self.fbo1_pass1.texture, w, h, -999.0, self.program_blur)
          self.fbo1_pass2.deactivate()

          # RENDER TO FBO2
          self.fbo2_pass2.activate()
          drawFullScreenTexturedQuadBlurred(self.fbo2_pass1.texture, w/2, h/2, -999.0, self.program_blur)
          self.fbo2_pass2.deactivate()

          # RENDER TO FBO3
          self.fbo3_pass2.activate()
          drawFullScreenTexturedQuadBlurred(self.fbo3_pass1.texture, w/4, h/4, -999.0, self.program_blur)
          self.fbo3_pass2.deactivate()

          # RENDER TO FBO4
          self.fbo4_pass2.activate()
          drawFullScreenTexturedQuadBlurred(self.fbo4_pass1.texture, w/8, h/8, -999.0, self.program_blur)
...
class ShockWaveEffect():
      def __init__(self, starsystem, center_x, center_y, x, y, z, time, dx, dy, dz, dtime):
          self.alive = True
...
def returnShockWavesDataInArrays(effect_list, vpCoordinate_x, vpCoordinate_y):
    ...
    for e in effect_list:
        center_list.append((e.center[0] - float(vpCoordinate_x)/VIEW_WIDTH , e.center[1] - float(vpCoordinate_y)/VIEW_HEIGHT))
        xyz_list.append((e.x, e.y, e.z))
        time_list.append(e.time)

    return np.array(center_list, dtype=np.float32), np.array(xyz_list, dtype=np.float32), np.array(time_list, dtype=np.float32)
...
class FBO():
      def __init__(self, width, height):
          ...
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,  width, height, 0, GL_RGBA, GL_INT, None) # no data transferred
          ...
          glBindFramebuffer(GL_FRAMEBUFFER, self.fbo)      # switch to our fbo so we can bind stuff to it
          glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, self.texture, 0)
          glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, self.depthbuffer)
