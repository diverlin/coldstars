...
### Global game classes
BLOOM_effect = bloomEffect(VIEW_WIDTH, VIEW_HEIGHT, program_blur, program_ExtractBrightAreas, program_Combine)
...
pygame.time.set_timer(UPDATE, int(1000.0/60/3))    # generate an event 60 times a second, and perform simulation update. this keeps the game running at the same speed in framerate-independent fashion.
...
def main():
 ...
 ss_active = global_STARSYSTEM_FREE_list[randint(0, len(global_STARSYSTEM_FREE_list)-1)]  
 ...
 BLOOM_effect.brightThreshold = player.starsystem.star.brightThreshold   # needs for nice star effect
 ...
 while RUNNING:
     ...
     if player.in_SPACE == True:
        ...
        RUNNING, vpCoordinate_x, vpCoordinate_y, turn_END, ships_info_SHOW, planets_info_SHOW, orbits_SHOW, show_RADAR, grapple_SELECTED, slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED, lb, rb, mx, my = KEYS_IN_SPACE.readControlKeys()
        ...
        if ss_active.screen_QUAKE_runtime_counter > 0:
           (vpCoordinate_x, vpCoordinate_y) = screenQuake((vpCoordinate_x, vpCoordinate_y), ss_active.screen_QUAKE_amlitudaDiv2)
        ...
        if timer < 0:
           ...
           ss_active.renderEntities(FBO0, FBO1, FBO2, BLOOM_effect, program_ShockWave, program_Shafts, vpCoordinate_x, vpCoordinate_y, lb, rb)
                   
           ss_active.renderRanges((slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED), show_RADAR, grapple_SELECTED)

           if INTERFACE_IN_SPACE.mouseInteraction((mx, my), lb, (vpCoordinate_x, vpCoordinate_y)) != True:
              ss_active.mouseInteraction(timer, (mx, my), (vpCoordinate_x, vpCoordinate_y), (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED), show_RADAR, grapple_SELECTED, (lb, rb))

           player.resetWeaponTargets((slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED))
        ...
        if timer > 0:
           ...
           ss_active.renderEntities(FBO0, FBO1, FBO2, BLOOM_effect, program_ShockWave, program_Shafts, vpCoordinate_x, vpCoordinate_y)
           ss_active.renderOrbits(orbits_SHOW)
           INTERFACE_IN_SPACE.render()
           drawVpCoord((vpCoordinate_x, vpCoordinate_y))
     ...
     if player.in_KOSMOPORT == True:
        ...
        if INTERFACE_IN_KOSMOPORT.angar_scr_SELECTED == True:
           ...
           if player.target_planet.port.angar.observed_ob == None:
              ...
              player.target_planet.port.angar.observed_ob.korpus.slotManagerLimitedControl(lb, (mx, my))
     ...
     if player.in_UNINHABITED_LAND == True:
        ...
        RUNNING, turn_END, lb, mx, my, goto_SPACE = KEYS_IN_KOSMOPORT.readControlKeys()
