...
class CommonForPlanets():
    ...
    def simpleOrbitFormation(self):
        ...
        self.turn_orbit_list_x, self.turn_orbit_list_y = getEllipseCoordinateRangeHighPrecision(self.orbit_centerx, self.orbit_centery, self.radius_A , self.radius_B, self.alpha_turn_step, self.fi)
        self.turn_orbit_list_len = len(self.turn_orbit_list_x)

        #########  init values  ####
        self.i = randint(int(self.turn_orbit_list_len / 2) - 2, int(self.turn_orbit_list_len / 2) + 2) * TURN_TIME
        self.angle = randint(0, 360)

    def detailedOrbitFormation(self):
        self.orbit_list_x, self.orbit_list_y = getEllipseCoordinateRangeHighPrecision(self.orbit_centerx, self.orbit_centery, self.radius_A , self.radius_B, self.speed, self.fi)
    ...
    def orbitVisualisation(self):
        ...
        if self.type == PLANET_ID:
           self.alpha_draw_step = self.alpha_turn_step * PLANET_DISTANCE_MIN / (max(self.radius_A , self.radius_B) * 1)
        if self.type == ASTEROID_ID:
           self.alpha_draw_step = self.alpha_turn_step * PLANET_DISTANCE_MIN / (max(self.radius_A , self.radius_B) * 4)

        self.draw_orbit_list_x, self.draw_orbit_list_y = getEllipseCoordinateRangeHighPrecision(self.orbit_centerx, self.orbit_centery, self.radius_A , self.radius_B, self.alpha_draw_step, self.fi)
        self.draw_orbit_list_len = len(self.draw_orbit_list_x)
        self.GL_ORBIT_LIST_ID = GlListCompileDirection(DOT_BLUE_TEX, self.draw_orbit_list_x, self.draw_orbit_list_y, self.draw_orbit_list_len, 1)

        self.GL_TURN_ORBIT_LIST_ID = GlListCompileDirection(DOT_RED_TEX, self.turn_orbit_list_x, self.turn_orbit_list_y, self.turn_orbit_list_len, 1, pointer_size = DOT_SIZE * 1.4)
...
class StarInstance(CommonForPlanets):
    ...
    def renderInfo(self):
        self.updateInfo()
        drawDynamicLabelList(text_background_tex, (self.points.center[0] + self.w, self.points.center[1] + self.h), self.info)
    ...
    def render3D_new(self):
        ...
        glUniform2f(glGetUniformLocation(self.program_multitex, 'displ'), self.offset1, self.offset2)
...
class PlanetInstance(CommonForPlanets):
    ...
    def __init__(self, name, model, texOb, program_light, race, population, subtype, starsystem, radius_A, radius_B, port, land):
        self.gl_list       = model.gl_list
        ...
        self.speed = randint(PLANET_SPEED_MIN, PLANET_SPEED_MAX)/float(radius_A/1000.0)
    ...
    def updateInfo(self):
        if self.subtype == PLANET_INHABITED_ID:
           self.info = [self.name, 'class: ' + self.returnPlanetClassStr(), 'race: ' + self.returnRaceStr(), 'size' + str(self.size), 'population: ' + str(self.population), 'speed: ' + str(int(self.speed)), "landed ships:" + str(len(self.port.angar.ship_list))]
        else:
           self.info = [self.name, 'class: ' + self.returnPlanetClassStr(), 'size' + str(self.size), 'speed: ' + str(int(self.speed))]
    ...
    def renderInfo(self):
        self.updateInfo()
        drawDynamicLabelList(text_background_tex, (self.points.center[0] + self.w/2, self.points.center[1] + self.h/2), self.info)
        if self.race != None:
           drawTexturedRect(self.port.goverment.face_texOb.texture, [self.points.center[0] + self.w/2 + 100, self.points.center[1] + self.h/2 + 100, 70, 70], -1.0)
    ...
    def render3D_new(self, vx, vy):
        ...
        glUniform4f(glGetUniformLocation(self.program_light, 'lightPos'), -vx, -vy, -200.0, 0.0)
        glUniform4f(glGetUniformLocation(self.program_light, 'eyePos'), -vx, -vy, -200.0, 0.0)
    ...
    def manageIncomingShip(self, ship):
        ...
        if self.subtype == PLANET_INHABITED_ID:
           H = returnFirstFreeSlotBySlotType(ANGAR_SLOT_ID, None, self.port.angar.slot_list)
           H.item = ship
    ...
    def landingRequest(self):
        if self.subtype == PLANET_INHABITED_ID:
           if len(self.port.angar.ship_list) < len(self.port.angar.slot_list):  # implement preservation of slots using counter
              return True
        if self.subtype == PLANET_UNINHABITED_ID:
           return True 
...
class SputnikInstance(CommonInstance, CommonForPlanets):
    ...
    def simpleSatelliteOrbitFormation(self):
        self.turn_orbit_list_x, self.turn_orbit_list_y = getSputnikCoordinateRange(self.planet.turn_orbit_list_x, self.planet.turn_orbit_list_y, self.radius, self.speed)
        self.turn_orbit_list_len = len(self.turn_orbit_list_x)

    def detaliedSatelliteOrbitFormation(self):
        self.orbit_list_x, self.orbit_list_y = getSputnikCoordinateRange(self.planet.orbit_list_x, self.planet.orbit_list_y, self.radius, self.speed)
        self.orbit_list_len = len(self.orbit_list_x)
...
class AsteroidInstance(CommonInstance, CommonForPlanets):
    ...
    def updateInfo(self):
        self.info = [self.name, 'mass: ' + str(self.mass), 'armor: ' + str(self.armor), 'speed: ' + str(self.speed) ]
    ...
    def hit(self, agressor, weapon):
        ...
        if self.alive == False:
           if self.alreadyInRemoveQueue == False:
              addExplosion(self)
              self.starsystem.addNumMinerals((self.points.center[0], self.points.center[1]), randint(2, 4))
...
class CommonForSpaceItems():
      ...
      def update(self, timer):
        if timer > 0:
           self.dx, self.dy = get_dX_dY_ToPoint((self.points.center[0], self.points.center[1]), (self.target_pos_x, self.target_pos_y) , self.step)
...
class Container(CommonInstance, CommonForSpaceItems):
      ...
      def __init__(self,  texOb, item, starsystem = None, pos_x = None, pos_y = None, target_x = None, target_y = None):
          CommonInstance.__init__(self, texOb.texture, (texOb.w, texOb.h))
          CommonForSpaceItems.__init__(self, pos_x, pos_y, target_x, target_y)
...
class Bomb(CommonInstance, CommonForSpaceItems):
      def __init__(self, texOb, armor, damage, radius, mass, speed):
          ...
          self.size = randint(7,10)   # improve depending on (damage + radius), + increase particles speed.

          self.updateInfo()


      def updateInfo(self):
          self.info = ['BOMB', 'damage:' + str(int(self.damage)), 'radius:' + str(int(self.radius)), 'armor:' + str(int(self.armor)), 'mass:' + str(int(self.mass))]
...
class rocketBulletInstance(CommonInstance):
    def __init__(self, texOb, owner, target, damage, size, armor, speed_init, speed_max, d_speed, live_time, angular_speed):
        self.alive = True
        self.alreadyInRemoveQueue = False
        self.in_SPACE = True
    ...
    def update(self, timer):
        if timer > 0:
           ...
           if self.target.alive == True:
             (self.dx, self.dy), self.angle_new = rocketWayCalc((self.points.center[0], self.points.center[1]), (self.target.points.center[0], self.target.points.center[1]), self.points.angle, self.angular_speed, self.step)
