...
class CommonForShip():
    ...
    def checkLanding(self):
        ship_landing_dist = lengthBetweenPoints((self.points.center[0], self.points.center[1]), (self.target_planet.points.center[0], self.target_planet.points.center[1]))
    ...
    def launchEvent(self):
        ...
        self.points.setCenter(self.target_planet.points.center[0], self.target_planet.points.center[1])
    ...
    def calculateDetaledWayToPosition(self):
        ...
        self.direction_list_x_part2, self.direction_list_y_part2, self.angle_list_part2 = follow_static_obj((last_pos_x, last_pos_y), (self.target_pos_x, self.target_pos_y) , self.speed)
        
        self.direction_list_x = self.direction_list_x_part1 + self.direction_list_x_part2
        self.direction_list_y = self.direction_list_y_part1 + self.direction_list_y_part2
    ...
    def calculateRoundDetaledWay(self, (pos_x, pos_y), (target_x, target_y), alpha, R, speed):  # not working yet
        direction_list_x = []
        direction_list_y = []
        angle_list = []
        ...
        # find colsest center
        len1 = lengthBetweenPoints((self.center1_x, self.center1_y), (target_x, target_y))
        len2 = lengthBetweenPoints((self.center2_x, self.center2_y), (target_x, target_y))
        ...
        #an1 =  
        len1 = lengthBetweenPoints((self.xk1, self.yk1), (self.points.midLeft[0], self.points.midLeft[1]))
        len2 = lengthBetweenPoints((self.xk2, self.yk2), (self.points.midLeft[0], self.points.midLeft[1]))
    ...
    def calculateTurnWayToPosition(self): 
        if self.target_pos_x != None and self.target_pos_y != None:
           self.turn_direction_list_x, self.turn_direction_list_y, _ = follow_static_obj((self.points.center[0], self.points.center[1]), (self.target_pos_x, self.target_pos_y) , self.turn_step)
    ...
    def calculateWayVisualisation(self):   
        if self.target_pos_x != None and self.target_pos_y != None:
           self.draw_direction_list_x, self.draw_direction_list_y, _ = follow_static_obj((self.points.center[0], self.points.center[1]), (self.target_pos_x, self.target_pos_y), self.draw_step)
    ...
    def updatePosition(self):
        if self.direction_list_END == False:
            if self.i < (self.len_direction_list - 1):
               self.points.setCenter(self.direction_list_x[self.i], self.direction_list_y[self.i])
    ...
    def updatePositionHidden(self):   # perform once per turn
        ...
        if self.turn_list_END == False:
            if self.turn_i < (self.len_turn_direction_list - 1):
               self.points.setCenter(self.turn_direction_list_x[self.turn_i], self.turn_direction_list_y[self.turn_i])
               self.turn_i += 1
            else:
               self.turn_list_END = True
    ...
    def setKorpus(self, korpus):
        ...
        if self.ableTo_DRIVE:
           ...
           self.drive_jet = driveTrailEffect(self, drive.pTexture, drive.pNum, self.size*drive.pSize, drive.pVelocity, drive.pAlphaInit, drive.pAlphaEnd, drive.pd_alpha)
        if self.ableTo_PROTECT:
           self.shield = shieldEffect(self, self.korpus.protector_slot.item.shield_texOb)
    ...
    def putAllItemsToSlots(self):
        ...
        for item in self.item_list:
            slot = returnFirstFreeSlotBySlotType(item.type, item.subtype, self.korpus.slot_list)
            if slot != None:
               self.putItemToSlot(item, slot) 
            else:
               otsec_slot = returnFirstFreeSlotBySlotType(OTSEC_SLOT_ID, None, self.korpus.otsec_slot_list)
               if otsec_slot != None:
    ...
    def putItemToSlot(self, item, slot):
        slot.item = item
        item.rect = pygame.Rect((slot.rect[0], slot.rect[1]), (slot.rect[2], slot.rect[3]))
        item.owner = self 
    ...
    def updateJumpAbility(self):
        ...
        self.bak = bak
        if (drive != None and drive.condition > 0) and (bak != None and bak.condition > 0):
    ...
    def updateProtectionAbility(self):
        ...
        if (protector != None and protector.condition > 0) and self.ableTo_ENERGIZE == True:
    ...
    def updateFireAbility(self):
        ...
        if len(self.armed_weapon_slot_list) != 0:
           self.average_fire_radius = sum_fire_radius/len(self.armed_weapon_slot_list)
    ...
    def buyFuel(self):
        if self.korpus.bak_slot.item != None:
           fuel_dif = self.korpus.bak_slot.item.fuel_max - self.korpus.bak_slot.item.fuel
    ...
    def energyRestoration(self):
        ...
        if self.korpus.energyBlock_slot.item != None:
           self.energy += self.energy_restoration_rate * self.korpus.energyBlock_slot.item.restoration
    ...
    def hyperJumpPreparation(self, target_starsystem):
        ...
        self.target_starsystem_dist = lengthBetweenPoints((self.starsystem.rectOnMap.centerx, self.starsystem.rectOnMap.centery), (self.target_starsystem.rectOnMap.centerx, self.target_starsystem.rectOnMap.centery))
        _y = self.target_starsystem.rectOnMap.centery - self.starsystem.rectOnMap.centery
        _x = self.target_starsystem.rectOnMap.centerx - self.starsystem.rectOnMap.centerx
        self._a = atan2(_x, _y)
        self.jump_pos_x, self.jump_pos_y = returnGeneratedTargetCoordinates(800, self._a, (0, 0))

    def navigateHyperJumpPoint(self):
        if abs(self.points.center[0] - self.jump_pos_x) < 10 and abs(self.points.center[1] - self.jump_pos_y) < 10:
           print self.name
           return True
        else:
           return False
    ...
    def hyperJumpEvent(self):
        if self.alive == True:
           if self.ableTo_JUMP:
              if self.target_starsystem_dist < self.korpus.drive_slot.item.hyper and self.target_starsystem_dist < self.korpus.bak_slot.item.fuel:
                 ...
                 self.red, self.green, self.blue = self.starsystem.red, self.starsystem.green, self.starsystem.blue
                 return True
        return False
    ...
    def setWeaponsToTarget(self, target, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED) = (True, True, True, True, True)):
        ...
        ship_target_dist = lengthBetweenPoints((self.points.center[0], self.points.center[1]), (target.points.center[0], target.points.center[1]))
        for w_slot in self.armed_weapon_slot_list:
            if w_slot.SELECTED == True and w_slot.target == None and ship_target_dist <= w_slot.item.radius:
    ...
    def weaponTargetAchievable(self, w_slot):
        ...
        if (w_slot.target.alive == False) or (w_slot.target.in_SPACE == False) or (w_slot.target.starsystem.name != self.starsystem.name):
            w_slot.target = None
            return False  
        else:   
           ship_slotTarget_dist = lengthBetweenPoints((self.points.center[0], self.points.center[1]), (w_slot.target.points.center[0], w_slot.target.points.center[1]))
    ...
    def resetWeaponTargets(self, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED)):
    ...
    def checkGrabQueue(self):   
        ...
        for item in self.korpus.grapple_slot.item.grapple_list:
            ship_item_dist = lengthBetweenPoints((self.points.center[0], self.points.center[1]), (item.points.center[0], item.points.center[1]))
            if ship_item_dist > self.korpus.grapple_slot.item.radius or item.alive == False or item.in_SPACE == False:
    ...
    def grabExecution(self):
        ...
        # in DYNAMIC
        for item in self.korpus.grapple_slot.item.grapple_list:
            item.target_pos_x, item.target_pos_y = self.points.center[0], self.points.center[1]  ##
            item.speed = self.korpus.grapple_slot.item.speed   
            item.stepCalculation()   
            if abs(self.points.center[0] - item.points.center[0]) < 20 and abs(self.points.center[1] - item.points.center[1]) < 20 and item.is_COLLECTED == False:
    ...
    def takeToOtsec(self, ob):
        ...
        if ob.type != CONTAINER_ID and ob.subtype == MINERAL_id:
           ...
           if slot != None:
              ...
              slot = returnFirstFreeSlotBySlotType(OTSEC_SLOT_ID, None, self.korpus.otsec_slot_list)
              if slot != None:
             ...
             if ob.subtype == EQUIPMENT_ID or ob.subtype == WEAPON_ID:
                ...
                if slot != None:
                 ...
                 if slot != None:
                  ...
                  slot = returnSlotWithGoodsBySubtype(ob.item.subtype, self.korpus.otsec_slot_list)
                  if slot != None:
    ...
    def dropToSpace(self, item):
        ...
        elif item.type == EQUIPMENT_ID or item.type == WEAPON_ID or item.type == MODULE_ID:
           texOb = TEXTURE_MANAGER.container_texOb_list[0]
        ...
        ob.target_pos_x, ob.target_pos_y = returnGeneratedTargetCoordinates(randint(50, 100), randint(0, 360) / 57.0, (ob.points.center[0], ob.points.center[1]))
    ...
    def updateInfo(self):
        ...
        self.info.append('armor:' +  str(int(self.korpus.armor)) + '/' + str(int(self.korpus.armor_max)))
        self.info.append('space all/free:' + str(int(self.korpus.space)) + '/' + str(int(self.korpus.space - self.mass)))
    ...
    def returnProtectionStr(self):
        # used in updateInfo
        if self.ableTo_PROTECT:
           return str(int(self.korpus.protector_slot.item.protection)) + '+' + str(int(self.korpus.protection))
        else:
           return str(int(self.korpus.protection))

    def renderInfo(self):
        self.updateInfo()
        drawDynamicLabelList(text_background_tex, (self.points.center[0] + self.points.w/2, self.points.center[1] + self.points.h/2), self.info)
        drawTexturedRect(self.face_texOb.texture, [self.points.center[0] + self.points.w, self.points.center[1] + self.points.h, 70, 70], -1.0)
    ...
    def hit(self, agressor, weapon, rate = 1):
        if agressor.type == SHIP_ID:
           damage = int(agressor.skill.attack/self.skill.defence * (1 - self.protection / 100.0) * weapon.damage)
        else:
           damage = int(100.0/self.skill.defence * (1 - self.protection / 100.0) * weapon.damage)
        self.korpus.armor -= damage * rate

        if self.korpus.protector_slot.item != None and self.energy > 0:   # improve by repalce ableToProtect
        ...
        if self.korpus.armor <= 0:
           ...
           self.starsystem.addContainers((self.points.center[0], self.points.center[1]), item_drop_queue)
    ...
    def renderInKosmoport(self, slot_rect):  
        drawTexturedRect(self.texture, [slot_rect.centerx - self.w/2, slot_rect.centery - self.h/2, self.w, self.h], -1.0)     
    ...
    def renderInSpace(self):
        ...
        if self.ableTo_GRAB == True:    
           for i in self.korpus.grapple_slot.item.grapple_list:
               len, angle_radian = returnLengthAngleBetweenPoints((self.points.center[0], self.points.center[1]), (i.points.center[0], i.points.center[1]))
               angle_degree = angle_radian * DEGREES_IN_RADIAN
               drawLine(grapple_jet_tex, (self.points.center[0], self.points.center[1], -2), len, angle_degree, 6)

        # render entity quad
        glBindTexture(GL_TEXTURE_2D, self.texture)
        drawQuadPer2DVertex(self.points.bottomLeft, self.points.bottomRight, self.points.topRight, self.points.topLeft, -3)
