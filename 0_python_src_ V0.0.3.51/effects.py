...
class tinyExplosionEffect():
      def __init__(self, texOb, starsystem, center, num_particles, pSize, velocity_max, alpha_init, alpha_end, d_alpha):
          self.alive = True
          ...
          self.particles_list = []
          for i in xrange(1, num_particles):
              simple_particle = particleForTinyExplosionEffect(self.center, velocity_max, alpha_init, alpha_end, randint(3,6)*0.01)
...
class explosionEffect():
      def __init__(self, texOb, starsystem, center, num_particles, pSize, d_pSize, velocity_max, d_velocity, alpha_init, alpha_end, d_alpha):
          self.alive = True
          self.alreadyInRemoveQueue = False
          ...
          self.particles_list = []
          for i in xrange(1, num_particles):
              simple_particle = particleForExplosionEffect(self.center, pSize, d_pSize, velocity_max, d_velocity, randint(70,100)*0.01, alpha_end, d_alpha, i)
              self.particles_list.append(simple_particle)
...
class particleForExplosionEffect():
      def __init__(self, center, pSize, d_pSize, velocity_max, d_velocity, alpha_init, alpha_end, d_alpha, num):
          self.alive = True
...
class driveTrailEffect():
      def __init__(self, ob, texOb, num_particles, pSize, velocity, alpha_init, alpha_end, d_alpha):
          self.texture = texOb.texture
          self.ob = ob
          ...
          self.particles_list = []
          for i in xrange(1, num_particles+1):
              simple_particle = particleForDriveTrailEffect(ob, alpha_init, alpha_end, d_alpha, i)
              self.particles_list.append(simple_particle)
...
class lazerEffect(CommonInstance):
      ...
      def updateAngleAndLen(self):
          ...
          self.ex = sin(DEGREES_90_IN_RADIAN - self.angle_radian) * self.Len + self.owner.points.center[0]
...
###                                                    texture,             starsystem,      center,  num_particles, pSize,              velocity_max, alpha_init, alpha_end, d_alpha):
      ...
      def _renderDynamicLineFrame(self):
          drawLine(self.texture_ID, (self.p1[0], self.p1[1], -2), self.Len, self.angle_degree, self.h/2)

      def _renderDynamicLineFramesLoop(self):
          drawLine(self.texture_ID[self._tex_frame], (self.p1[0], self.p1[1], -2), self.Len, self.angle_degree, self.h/2)
          self.updateAnimationFrameLoop()
...
def addExplosion(ob):
    ...
    if ob.size < 4:
       ...
       num_particles2 = 50
       pSize2         = 25 * (ob.size-1)
       pe = explosionEffect(TEXTURE_MANAGER.returnParticleTexObBy_ColorID(YELLOW_COLOR_ID), ob.starsystem, ob.points.center, num_particles2, pSize2, d_pSize, velocity_max, d_velocity, alpha_init, alpha_end, d_alpha)
       ob.starsystem.effect_EXPLOSION_list.append(pe)

       num_particles2 = 100
       pSize2         = 25 * (ob.size-2)
       pe = explosionEffect(TEXTURE_MANAGER.returnParticleTexObBy_ColorID(RED_COLOR_ID), ob.starsystem, ob.points.center, num_particles2, pSize2, d_pSize, velocity_max, d_velocity, alpha_init, alpha_end, d_alpha)
       ob.starsystem.effect_EXPLOSION_list.append(pe)
