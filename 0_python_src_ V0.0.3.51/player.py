...
class PlayerShip(CommonForShip):
    ...
    def GlListCompileRadius(self):
        for w_slot in self.armed_weapon_slot_list:
            self.w_slot_list_x, self.w_slot_list_y = getCircleCoordinateRangeLowPrecision(self.points.center[0], self.points.center[1], w_slot.item.radius)
            w_slot.GL_LIST_slot_ID = GlListCompileDirection(DOT_RED_TEX, self.w_slot_list_x, self.w_slot_list_y, len(self.w_slot_list_x), 1)

        if self.korpus.grapple_slot.item != None:
           self.grapple_list_x, self.grapple_list_y = getCircleCoordinateRangeLowPrecision(self.points.center[0], self.points.center[1], self.korpus.grapple_slot.item.radius)
           self.GL_LIST_grapple_ID = GlListCompileDirection(DOT_BLUE_TEX, self.grapple_list_x, self.grapple_list_y, len(self.grapple_list_x), 1)

        if self.korpus.radar_slot.item != None:
           self.radar_slot_list_x, self.radar_slot_list_y = getCircleCoordinateRangeLowPrecision(self.points.center[0], self.points.center[1], self.korpus.radar_slot.item.radius)
           self.GL_LIST_radar_ID = GlListCompileDirection(DOT_BLUE_TEX, self.radar_slot_list_x, self.radar_slot_list_y, len(self.radar_slot_list_x), 1)


    def renderRadius(self, (slot_1_SELECTED, slot_2_SELECTED, slot_3_SELECTED, slot_4_SELECTED, slot_5_SELECTED), grapple_SELECTED, show_RADAR):
    ...
    def renderIcons(self):
        if self.korpus.grapple_slot.item != None:
           for item in self.korpus.grapple_slot.item.grapple_list:
               drawTexturedRect(grabbing_ICON_tex, [item.points.center[0], item.points.center[1], 20, 20], -1.0)
        i = 0
        for w_slot in self.reloaded_weapon_list:
            if w_slot.target != None:
               drawTexturedRect(w_slot.item.texture_ID, [w_slot.target.points.center[0] - w_slot.target.w/2 + 23*i, w_slot.target.points.center[1] + w_slot.target.h/2, 20, 20], -1.0)
               i += 1
    ...
    def setGrabItem(self, item):
        if self.ableTo_GRAB == True:         
           ...
           if ship_item_dist <= self.korpus.grapple_slot.item.radius:        # check if item lays in range of grapple access area
              for marked_item in self.korpus.grapple_slot.item.grapple_list: # check if item is already in the grapple target list, if yes it shall be removed
                  if marked_item == item:
                     self.addToGrappleRemoveQueue(item)
                     skeep_grabbing = True
                     break

              if (item.mass <= self.korpus.grapple_slot.item.strength) and (len(self.korpus.grapple_slot.item.grapple_list) < self.korpus.grapple_slot.item.maxNumItem) and skeep_grabbing == False:  # check the rest condition for grabbing and if yes, grab it
