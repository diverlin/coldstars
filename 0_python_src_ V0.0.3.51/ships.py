...
class NPC(CommonForShip):
    ...
    def sensorium(self, asteroid_list, mineral_list, container_list):
        ...
        for a in asteroid_list:
            ship_asteroid_dist = lengthBetweenPoints((self.points.center[0], self.points.center[1]), (a.points.center[0], a.points.center[1]))
        ...
        for c in container_list:
            ship_container_dist = lengthBetweenPoints((self.points.center[0], self.points.center[1]), (c.points.center[0], c.points.center[1]))
        ...
        for m in mineral_list:
            ship_mineral_dist = lengthBetweenPoints((self.points.center[0], self.points.center[1]), (m.points.center[0], m.points.center[1]))
        ...
        self.visible_asteroid_with_distance_tuples = sorted(visible_asteroid_with_distance_tuples, key=itemgetter(1))
        self.visible_container_with_distance_tuples = sorted(visible_container_with_distance_tuples, key=itemgetter(1))
        self.visible_mineral_with_distance_tuples = sorted(visible_mineral_with_distance_tuples, key=itemgetter(1))


    def thinkInSpaceRace0(self):
        ##########################################################
        if self.race != RACE_6_ID and self.race != RACE_7_ID:                           # DEPR
           if len(self.QUEST_TASK_queue) == 0: # and len(self.NEEDS_TASK_queue) == 0:      # DEPR
              self.questManager()
        ##########################################################
        ...
        if self.quest_ob != None: 
           if self.starsystem != self.quest_ob.starsystem:
              if self.task_id_being_exec != LANDING_task_id and self.task_id_being_exec != HYPER_JUMP_task_id:
    ...
    def taskManager(self):
        ...
        if len(self.NEEDS_TASK_queue) !=0:
           self.taskExecution_inDynamic, self.taskExecution_inStatic = self.initAndReturnNeedsExecutionFunction()
        elif len(self.QUEST_TASK_queue) != 0:
           self.taskExecution_inDYnamic, self.taskExecution_inStatic = self.initAndReturnQuestExecutionFunction()
    ...
    def initAndReturnNeedsExecutionFunction(self):
        ...
        if self.task_id_being_exec == HYPER_JUMP_task_id:
           ...
           return self.hyperJumpTaskExecution_inDynamic, self.hyperJumpTaskExecution_inStatic 
             ...
             return self.landingTaskExecution_inDynamic, self.landingTaskExecution_inStatic  

        elif self.task_id_being_exec == LAUNCHING_task_id:
             self.launchingTaskInit() 
             return self.launchingTaskExecution_inDynamic, self.launchingTaskExecution_inStatic
    ...
    def destroyAlienTaskExecution_inStatic(self):
        if self.target_ship == None:
           ...
           self.calculateWayToCoord((self.target_ship.points.center[0], self.target_ship.points.center[1]))
    ...
    def hyperJumpTaskInit(self):
       ...
       # self.functions_inDynamic_queue = [self.navigateHyperJumpPoint, self.hyperJumpInEffect, self.hyperJumpEvent, self.hyperJumpOutEffect, self.cleaningUpAfterHyperJump]
        self.functions_inDynamic_queue = [self.navigateHyperJumpPoint, self.hyperJumpEvent, self.cleaningUpAfterHyperJump]
    ...
    def landingTaskExecution_inStatic(self):
        self.calculateWayToCoord((self.target_planet.points.center[0], self.target_planet.points.center[1]))  # refresh target coord because of planet movement
    ...
    def thinkAtInhabitedPlanet(self):
        ...
        for reason in self.landing_reason_id_list:
            ...
            if reason == SELL_NEEDED_id:
               print "Before ___ free_space/speed", int(self.korpus.space - self.mass),'/', self.speed                  # debug
               for slot in self.korpus.otsec_slot_list:
                   if slot.item != None:
                      if slot.item.type == CONTAINER_ID:
                         if slot.item.item.type == GOODS_id:
                            earn_money = self.target_planet.kosmoport.shop.sell(slot.item.item)
               ...
               print "After ___ free_space/speed", int(self.korpus.space - self.mass),'/', self.speed                   # debug
        ...
        if len(self.landing_reason_id_list) == 0 and self.task_id_being_exec != LAUNCHING_task_id:
    ...
    def grabbingContainerTaskInit(self):
        for tuple in self.visible_container_with_distance_tuples:
            ...
            if c.mass < self.korpus.grapple_slot.item.strength:
               ...
               self.calculateWayToCoord((c.points.center[0] - 10, c.points.center[1] - 10))
               break
    ...
    def grabbingMineralTaskInit(self):
        for tuple in self.visible_mineral_with_distance_tuples:
            ...
            if m.mass < self.korpus.grapple_slot.item.strength:
               ...
               self.calculateWayToCoord((m.points.center[0] - 10, m.points.center[1] - 10))
               break
    ...
    def checkGrabTaskCondition(self):
        if self.grapping_main_target != None:
           if self.grapping_main_target.alive == False or self.grapping_main_target.in_SPACE == False: 
              self.grapping_main_target = None
           ...
           if self.grapping_main_target != None:
              ...
              if self.see_CONTAINER == True:
                 for (c, dist) in self.visible_container_with_distance_tuples:
                     if len(self.korpus.grapple_slot.item.grapple_list) == self.korpus.grapple_slot.item.maxNumItem:
                        break
